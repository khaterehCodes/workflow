...
function EditUsersPage() {
    ...
    return (
        <div>
            ...
            <div className="p-6 w-[1104px] h-[498px] mx-auto bg-white rounded-[12px] shadow">
                <p className=" text-[#27272A] text-[18px] mb-6">{isEdit ? "Editer un compte" : "Ajouter un utilisateur"}</p>
                <form onSubmit={formikSubmit} className="grid grid-cols-2 font-normal text-[16px] gap-x-4 gap-y-6">
                    ...
                    <hr className="col-span-2 border-t w-full mt-[30px]" />
                    <button
                        type="submit"
                        disabled={isSubmitDisabled || isSubmitting}
                        className={`col-span-2 w-[103px] mt-[0px] h-[48px] py-2 rounded-[10px] text-white font-bold ${isSubmitDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
                            }`}>
