...
function ConfigurationSideBar() {
  ...
  const isItemActive = (item) => {
    return item.paths.some(path => location.pathname === path);
  };

  return (
    <div className="mt-7 w-[216px]">
      <div className="flex items-center justify-between px-7 h-[48px] cursor-pointer"
        onClick={() => setIsOpen(!isOpen)}>
        <div className="flex items-center space-x-2">
          <FiSettings size={20} className={isOpen ? "text-white" : "text-gray-400"} />
          <P className={`text-[16px] ${isOpen ? "font-medium text-white" : "font-normal text-gray-400"}`}> Configuration</P>
        </div>
        <div className="ml-12">
          {isOpen ? (
          ...
          {items.map((item, index) => {
            ...
            return (
              <Link key={index} to={item.route} className="no-underline">
                <div className={`cursor-pointer px-3 py-2 rounded-[10px] transition-colors 
